...
Note: it currently only support JPG, JPEG and PNG Images with expected future support for other formats
...
- **Local Import**: importing an image from device through selecting and image and the behavior (cut → remove the image from the source folder after importing or copy → keep the image in the source folder after importing) - the default folder to open to select the image and the default behavior can be configured through the plugin's settings. **Note**: the plugin has to access files outside of the vault for that feature.
